...
if __name__ == "__main__":
    ...
    detector._load_data("enron_spam_data.csv")
    detector._train()
    detector._evaluate()
    text = "Hello Jack! I hope you're doing well i'm contacting you to inform you about the meeting we are holding in hostin this week ,My Regards."
    result = detector._predict(text)
    print(detector._format_output(result))

    pickle.dump(detector, open("model\\spam_detector.pkl", "wb"))
    pickle.dump(detector.vectorizer, open("model\\vectorizer.pkl", "wb"))
    pickle.dump(detector.model, open("model\\logistic_model.pkl", "wb"))
